...
def update_stats(stats, ground_truth, predictions, shape_id, start_class, end_class):
    ...
    for i in range(start_class, end_class):
        igt = (ground_truth == i) # n 由布尔值组成的列表
        ipd = (predictions == i)
        union = np.sum(igt | ipd) # 并集 数量
        intersection = np.sum(igt & ipd) # 交集 数量

        if union == 0:
            iou += 1
        else:
            iou += intersection / union # 零件iou之和
    iou /= (end_class - start_class) # 零件iou的平均作为该形状的iou
